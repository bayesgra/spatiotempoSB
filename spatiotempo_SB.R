...
#The whole Shebang!
STSB<-function(y,x=NA,z,t=NULL,DOF=1,mx.sige=1,mx.sigs=1,n.terms=50,pred=F,x_new,z_new,t_new=NULL,
...
#  for(k in 1:n.terms){vs[,k]<-onevs(z,rho,knot[,k],v[k])}
  ...
  for(i in 1:runs){
    ...
    mn<-COV%*%t(x)%*%diag(taue[g])%*%matrix((y-mu[g]),nrow=n)
    beta<-mn+t(chol(COV))
    r<-y-x%*%beta
    ...
    for(j in 1:2){for(k in 1:n.terms){
      ...
      if(canknot[j,k]> 0 & canknot[j,k]<1){
        canvs[,k]<-onevs(z,rho,canknot[,k],v[k],kernel=kernel,t=t,a=1,beta=betagn)
        canprobs<-makeprobs(canvs)
        MHrate<-sum(log(canprobs[cbind(1:n,g)])-
                      log(probs[cbind(1:n,g)]))     
        if(runif(1,0,1)<exp(MHrate)){
